...
def run_lin_nlin_smac_params():
    ...
    for linearity, linearity_str in zip([True, False],['linear', 'nonlinear']):
        for mechanism in mechanisms:
            ...
            smac_solution = np.load('./results/linear_nonlinear/' + linearity_str + '_'+ mechanism + '.npy')
            print(smac_solution)
            minimum = smac_solution['min']
            maximum = minimum + smac_solution['max_minus_min']
            if maximum >= 1:
            ...
            for function, function_str in zip(train_functions, train_functions_names):        
                ...
                function_costs = []
                for _ in range(metaheuristic_runs):
                    function_costs.append(function_cost(function, variables_range, variables_bounded, acor_mechanism, minimum, maximum, linear_control, acor_iterations))
